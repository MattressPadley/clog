...
## Log Level Hierarchy in Multi-Library Projects
...
CLog is designed for compile-time configuration to achieve optimal performance. The recommended approach is to configure via CMake, with preprocessor defines as an alternative. For projects using multiple libraries, see [Log Level Hierarchy in Multi-Library Projects](#log-level-hierarchy-in-multi-library-projects) above for proper override patterns.
